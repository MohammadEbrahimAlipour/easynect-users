...
const StatsChartSide = ({
  ...
  const viewData = useMemo(() => {
    if (viewFetch.response?.data) return viewFetch.response?.data;
    return null;
  }, [viewFetch.response]);

  const connectionData = useMemo(() => {
    if (connectionFetch.response?.data) return connectionFetch.response?.data;
    return null;
  }, [connectionFetch.response]);

  const convertData = useMemo(() => {
    if (convertFetch.response?.data) return convertFetch.response?.data;
    return null;
  }, [convertFetch.response]);

  const shareData = useMemo(() => {
    if (shareFetch.response?.data) return shareFetch.response?.data;
    return null;
  }, [shareFetch.response]);
