...
const CreatedOrder = () => {
  ...
  const handleSubmit = (event) => {
    ...
    if (Number(type) > 0) {
      ...
      axios.post(
        API_ROUTES.ADD_INVOICE_FORM_FIELD(id, formId),
        fieldValues,
        {
          headers: {
      ...
      .catch((error) => {
        console.error("Error adding invoice form field:", error);
        toast.error('افزودن فیلد با مشکل مواجه شد');
      });
  ...
  const handleInnerItemClick = (item) => {
    axios
      .get(API_ROUTES.LEAD_CAPTURE_STORE_ID(item), {
        headers: {
          Authorization: `Bearer ${accessToken.accessToken}`,
          "Accept-Language": "fa",
        },
  ...
  return (
    <>
      ...
      <HeaderTwo />
      <Layout>
        <h3 className="font-semibold mb-6">ساخت فرم سفارش</h3>
        ...
        {selectedFields.map((field, index) => (
          <div
            ...
            className="bg-lightMenu rounded-lg  mb-3 border-2 box-border flex justify-between
              items-center"
          >
            <div className="flex justify-between w-full items-center  py-3" onClick={() => handleInnerItemClick(field.id)}>
              <div>
                {/* input to send */}
                <p
                  id={`create-${index}`}
        ...
        >
          ...
          <div
            ...
            bg-white transition-all ease-in-out duration-300 max-h-[150px] overflow-y-scroll ${showList ? "" : "opacity-0 pointer-events-none"
              }`}
          >
