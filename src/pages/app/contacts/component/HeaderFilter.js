import { ContactsFilterIcon, ContactsImportIcon, SearchIcon } from "@/components/Icons";
...
export default function ({
	...
	const contractPageList = useMemo(() => {
		if (contractPageFetch.response?.data && Array.isArray(contractPageFetch.response?.data))
			return contractPageFetch.response?.data
		return []
	}, [contractPageFetch.response?.data]);

	const handlerSearch = useCallback(debounce((text) => {
		onSearchQuery(text);
	}, 800), [onSearchQuery])
